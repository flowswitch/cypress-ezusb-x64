...
#define CHUNKLENGTH 512
      ...
      case IOCTL_Ezusb_GET_PIPE_INFO:
		...
		//HACK: adjust 32/64 incompatibility
		if (length>0x10) //contains 64bit handle part
		{
			*(PUSHORT)pch -= 4;
			RtlCopyMemory(pch+0x0C, pch+0x10, length-0x10);
			length-=4;
		}
#endif
         }
      ...
      case IOCTL_Ezusb_GET_STRING_DESCRIPTOR:
         {
            ...
            //BUG: original code doesn't check inputBufferLength correctly, keep for compatibility
            //LuLaUsb passes wrong length here
    ...
    )
{
    ...
    if (urb)
    {
        ...
        for (i=0; i < 5120 / CHUNKLENGTH; i++)
        {
            ...
            CompareCount = (ULONG)RtlCompareMemory(buffer1,buffer2,CHUNKLENGTH);
